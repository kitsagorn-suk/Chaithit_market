...
namespace Chaithit_Market.Services
{
    public class GetService
    {
        ...
        public GetTotalHistoryPaidBillAdminModel GetTotalHistoryPaidBillAdminService(string authorization, string lang, string platform, int logID, SearchHistoryAdminBillDTO searchHistoryAdminBillDTO)
        {
            if (_sql == null)
            {
                _sql = SQLManager.Instance;
            ...
            GetTotalHistoryPaidBillAdminModel value = new GetTotalHistoryPaidBillAdminModel();
            try
            {
                value.data = new TotalHistoryPaidBillAdminModel();

                ValidationModel validation = ValidationManager.CheckValidation(1, lang, platform);

                if (validation.Success == true)
                {
                    value.data = _sql.GetTotalHistoryPaidBillAdmin(searchHistoryAdminBillDTO);
                }
                else
                {
                    _sql.UpdateLogReceiveDataError(logID, validation.InvalidMessage);
            ...
            {
                LogManager.ServiceLog.WriteExceptionLog(ex, "GetTotalHistoryPaidBillAdminService:");
                if (logID > 0)
                {
                    _sql.UpdateLogReceiveDataError(logID, ex.ToString());
                }
        ...
        public GetHistoryPaidBillUserNoPageModel SearchHistoryPaidBillUserNopageService(string authorization, string lang, string platform, int logID, SearchHistoryUserBillDTO searchHistoryUserBillDTO)
        {
            if (_sql == null)
            {
                _sql = SQLManager.Instance;
            ...
            GetHistoryPaidBillUserNoPageModel value = new GetHistoryPaidBillUserNoPageModel();
            try
            {
                value.data = new List<HistoryPaidBillUserNoPageModel>();

                ValidationModel validation = ValidationManager.CheckValidation(1, lang, platform);

                if (validation.Success == true)
                {
                    value.data = _sql.SearchHistoryPaidBillUserNoPage(searchHistoryUserBillDTO);
                }
                else
                {
                    _sql.UpdateLogReceiveDataError(logID, validation.InvalidMessage);
            ...
            {
                LogManager.ServiceLog.WriteExceptionLog(ex, "SearchHistoryPaidBillUserNopageService:");
                if (logID > 0)
                {
                    _sql.UpdateLogReceiveDataError(logID, ex.ToString());
                }
